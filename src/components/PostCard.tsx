...
export default function PostCard({ post, onPostDeleted, onPostClick, showComments = false, isPriority = false }: PostCardProps) {
  ...
  return (
    <div className="relative">
      <div
        className={`w-full bg-white ${showCommentDrawer ? 'rounded-t-3xl shadow-none border-b-0' : 'rounded-none sm:rounded-3xl shadow-sm'} border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 relative ${pathname.includes('/post/') ? 'cursor-default' : 'cursor-pointer'
          }`}
        onClick={(e) => {
